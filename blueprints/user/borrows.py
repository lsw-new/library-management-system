...
# 定义 `_parse_return_date` 函数，封装一段可复用的后端处理流程。
...
def register_borrow_routes(bp: Blueprint) -> None:
    ...
    # 装饰器行，为下面的函数或类附加路由、权限、限流或其他框架行为。
    @login_required
    # 定义 `borrow_book` 函数，封装一段可复用的后端处理流程。
    ...
    # 装饰器行，为下面的函数或类附加路由、权限、限流或其他框架行为。
    @login_required
    # 定义 `cancel_reservation` 函数，封装一段可复用的后端处理流程。
    ...
    # 装饰器行，为下面的函数或类附加路由、权限、限流或其他框架行为。
    @login_required
    # 定义 `borrow_records` 函数，封装一段可复用的后端处理流程。
    def borrow_records():
        # 执行变量、配置或对象属性赋值，保存后续逻辑需要的数据。
    ...
    # 装饰器行，为下面的函数或类附加路由、权限、限流或其他框架行为。
    @login_required
    # 定义 `borrow_records_status` 函数，封装一段可复用的后端处理流程。
