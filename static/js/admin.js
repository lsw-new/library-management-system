...
function switchAdminTab(btn, tabKey) {
    ...
    .then(htmlText => {
        ...
        window.history.replaceState({}, '', `${baseUrl}?tab=${tabKey}`);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addBookForm) {
        addBookForm.addEventListener('submit', function(e) {
            ...
            fetch('/book/add', { method: 'POST', body: formData })
                .then(res => res.json())
                .then(data => {
                    btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editBookForm) {
        editBookForm.addEventListener('submit', function(e) {
            ...
            const formData = new FormData(form);
            fetch(`/book/edit/${currentEditBookId}`, { method: 'POST', body: formData })
                .then(res => res.json())
                .then(data => {
                    btn.disabled = false;
...
function deleteCurrentBook() {
    ...
    showConfirm('删除图书', '确定要删除这本图书吗？此操作无法撤销。', () => {
        fetch(`/book/delete/${currentEditBookId}`, { method: 'POST' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    showToast(data.message, 'success');
...
function approveReservation(recordId) {
    ...
    showConfirm('同意领取', '确认同意该用户领取图书吗？', () => {
        fetch(`/admin/approve_reservation/${recordId}`, { method: 'POST' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    showToast(data.message, 'success');
...
function rejectReservation(recordId) {
    ...
    showConfirm('拒绝领取', '确认拒绝该用户领取图书吗？库存将自动恢复。', () => {
        fetch(`/admin/reject_reservation/${recordId}`, { method: 'POST' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    showToast(data.message, 'success');
...
function adminReturnBook(recordId) {
    ...
    showConfirm('确认归还', '确认该图书已归还入库吗？', () => {
        fetch(`/admin/return_book/${recordId}`, { method: 'POST' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    showToast(data.message, 'success');
...
function buildUserCard(u) {
    ...
    return `
        <article class="online-user-card" data-uid="${u.user_id}" data-utype="${u.user_type}">
            <div class="flex items-center gap-3 mb-3">
                <div class="relative flex-shrink-0">
                    <div class="w-11 h-11 rounded-xl flex items-center justify-center font-bold text-base shadow-sm ${avatarClass}">${u.username.charAt(0).toUpperCase()}</div>
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-sm font-bold text-brand-deep truncate">${u.username}</p>
            ...
            <div class="flex items-center justify-between text-[11px] pt-3 border-t border-pink-50">
                <span class="font-mono text-brand-deep/50">${u.ip_address || '—'}</span>
                <span class="font-mono font-semibold text-brand-deep/70 online-last-seen">${u.last_seen}</span>
            </div>
            ${kickBtn}
        </article>`;
}
...
function openBanModal(userId, username) {
    ...
    overlay.innerHTML = `
        <div class="bg-white w-full max-w-md rounded-[2.5rem] p-7 md:p-8 shadow-2xl animate-in zoom-in-95 duration-300" role="dialog" aria-modal="true">
            <div class="flex flex-col items-center text-center">
                ...
                <h3 class="mb-2 text-2xl font-heading font-bold text-brand-deep">封禁账号</h3>
                ...
                <div class="w-full mb-4">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ${[5, 30, 60, 180, 720, 1440].map(m => `
                            <button type="button" data-mins="${m}" class="ban-quick px-3 py-2 text-xs font-bold rounded-xl border border-rose-100 text-rose-600 hover:bg-rose-50 hover:border-rose-300 transition-colors">
                ...
                <div class="w-full mb-6">
                    <label class="block text-[10px] font-bold text-brand-deep/45 uppercase tracking-widest mb-2 text-left">自定义时长（分钟）</label>
    ...
    overlay.querySelector('.ban-cancel').addEventListener('click', closeModal);
