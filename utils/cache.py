...
# 定义 `init_cache` 函数，封装一段可复用的后端处理流程。
...
# 定义 `get_redis` 函数，封装一段可复用的后端处理流程。
def get_redis():
    # 返回当前函数的结果或 HTTP 响应，并结束这一条执行路径。
    return _redis_client


# 定义 `cache_get` 函数，封装一段可复用的后端处理流程。
...
# 定义 `cache_set` 函数，封装一段可复用的后端处理流程。
...
# 定义 `cache_delete` 函数，封装一段可复用的后端处理流程。
...
# 定义 `cache_delete_pattern` 函数，封装一段可复用的后端处理流程。
