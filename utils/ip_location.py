...
# 逐行注释：定义 `_is_private` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：定义 `_lookup_ip_cn` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：定义 `_lookup_ip_api` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：定义 `get_ip_location` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：定义 `get_ip_location_cached` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：定义 `schedule_ip_lookup` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：空行，用于分隔 Python 代码块，提升代码阅读层次。
    ...
    # 逐行注释：定义 `_do_lookup` 函数，封装一段可复用的后端处理流程。
    def _do_lookup():
        # 逐行注释：异常保护入口，下面的逻辑可能失败，需要捕获并转成可控结果。
        try:
            # 逐行注释：执行变量、配置或对象属性赋值，保存后续逻辑需要的数据。
...
# 逐行注释：定义 `build_ip_location_map` 函数，封装一段可复用的后端处理流程。
