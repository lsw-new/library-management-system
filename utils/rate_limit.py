...
# 逐行注释：定义 `_get_redis` 函数，封装一段可复用的后端处理流程。
def _get_redis():
...
# 逐行注释：定义 `_rate_limit_identity` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：定义 `is_rate_limited` 函数，封装一段可复用的后端处理流程。
...
# 逐行注释：定义 `rate_limit` 函数，封装一段可复用的后端处理流程。
